...
export default function KategoriManagementPage() {
  const [list, setList] = useState<ApiKategori[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [mode, setMode] = useState<ModalMode>(null);
  const [target, setTarget] = useState<ApiKategori | null>(null);
  ...
  const [toggleTarget, setToggleTarget] = useState<ApiKategori | null>(null);
  const [showToggleSuccess, setShowToggleSuccess] = useState<ApiKategori | null>(null);
  const [newToggleAktif, setNewToggleAktif] = useState<boolean | null>(null);
  ...
  function openTambah() {
    setNamaInput("");
    setDescInput("");
    setWarnaPick(WARNA_OPTIONS[0].value);
    setTarget(null);
    setMode("tambah");
  }

  function openEdit(k: ApiKategori) {
    setNamaInput(k.nama_kategori);
    setDescInput(k.deskripsi ?? "");
    setWarnaPick(k.warna ?? WARNA_OPTIONS[0].value);
    setTarget(k);
    setMode("edit");
  }

  function openHapus(k: ApiKategori) {
    setTarget(k);
    setMode("hapus");
  }
  ...
  async function confirmSimpan() {
    setSubmitting(true);
    try {
      if (pendingMode === "tambah") {
        await api.createKategori({
          nama_kategori: namaInput.trim(),
          deskripsi: descInput.trim() || null,
          warna: warnaPick,
          aktif: true,
        });
      } else if (pendingMode === "edit" && target) {
        await api.updateKategori(target.id_kategori, {
          nama_kategori: namaInput.trim(),
          deskripsi: descInput.trim() || null,
          warna: warnaPick,
        });
  ...
  async function confirmHapus() {
    ...
    setSubmitting(true);
    try {
      await api.deleteKategori(target.id_kategori);
      setShowConfirm(null);
  ...
  async function confirmToggle() {
    ...
    const newAktif = !toggleTarget.aktif;
    setSubmitting(true);
    try {
      await api.updateKategori(toggleTarget.id_kategori, { aktif: newAktif });
      setNewToggleAktif(newAktif);
      setShowToggleSuccess(toggleTarget);
    ...
    <div className="p-6 space-y-5 min-h-full" style={{ backgroundColor: PAGE }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          ...
          { label: "NONAKTIF", value: loading ? "—" : list.length - totalAktif, color: "#ef4444" },
        ].map((s, i) => (
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  {["NAMA KATEGORI", "DESKRIPSI", "JUMLAH MENU", "STATUS", "AKSI"].map((h) => (
                    <th key={h} className="px-5 py-3.5 text-left text-[10px] font-bold tracking-widest" style={{ color: "#CBC3D7" }}>
                      {h}
                    </th>
                  ))}
                </tr>
                    ...
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2.5">
                          <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: k.warna ?? "#94a3b8" }} />
                          <span className="text-white font-semibold">{k.nama_kategori}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <span
                          ...
                          style={{ backgroundColor: `${k.warna ?? "#94a3b8"}18`, color: k.warna ?? "#94a3b8" }}
                        >
                          {k.jumlah_menu} menu
                        </span>
                      </td>
        ...
        >
          ...
          >
            ...
            <p className="text-sm" style={{ color: "#94a3b8" }}>
              Yakin ingin menghapus kategori <span className="font-bold text-white">{target.nama_kategori}</span>? Tindakan ini tidak dapat dibatalkan dan akan mempengaruhi{" "}
              <span className="font-bold" style={{ color: "#ef4444" }}>{target.jumlah_menu} menu</span> yang terdaftar.
            </p>
            <div className="flex items-center gap-2 pt-1">
              <button
                onClick={closeModal}
              ...
              <button onClick={handleHapus} className="flex-1 py-2 rounded-xl text-sm font-bold" style={{ backgroundColor: "#ef4444", color: "#fff" }}>
                Hapus
              </button>
            </div>
          </div>
      ...
      {showConfirm === "hapus" && target && (
        ...
        >
          ...
          >
            ...
            <div className="space-y-2">
              <h3 className="text-white font-bold text-lg leading-snug">Hapus Kategori Ini?</h3>
              <p className="text-sm leading-relaxed" style={{ color: "#94a3b8" }}>
                Kategori <span className="text-white font-semibold">{target.nama_kategori}</span> akan dihapus permanen dan mempengaruhi{" "}
                <span className="font-semibold" style={{ color: "#ef4444" }}>{target.jumlah_menu} menu</span> yang terdaftar.
              </p>
            </div>
            <div className="flex items-center gap-3 w-full pt-1">
              <button
        ...
        >
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm leading-relaxed" style={{ color: "#94a3b8" }}>
                Kategori <span className="text-white font-semibold">{toggleTarget.nama_kategori}</span> akan diubah menjadi{" "}
                <span className="font-semibold" style={{ color: ACCENT }}>
                  {toggleTarget.aktif ? "Nonaktif" : "Aktif"}
                </span>.
                {toggleTarget.aktif && (
                  <span className="block mt-1 text-xs" style={{ color: "#ef4444" }}>
                    Ini akan mempengaruhi {toggleTarget.jumlah_menu} menu yang terdaftar.
                  </span>
                )}
              </p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.6)" }}>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm leading-relaxed" style={{ color: "#94a3b8" }}>
                Kategori <span className="text-white font-semibold">{showToggleSuccess.nama_kategori}</span> kini berstatus{" "}
                <span className="font-semibold" style={{ color: ACCENT }}>
                  {newToggleAktif ? "Aktif" : "Nonaktif"}
                </span>.
              </p>
            </div>
            <button
              onClick={() => {
                setShowToggleSuccess(null);
                setNewToggleAktif(null);
              }}
